...
/**
 * ------------------------------------------------------------
 * runMain main関数を起動するためのスターター関数
 * ------------------------------------------------------------
 * 目的：
 *   ・必ず "main" エンジンでスクリプトを実行する
 *   ・BridgeTalk を使ってエンジンを切り替える
 *   ・2回目以降は evalFile を行わず高速実行（ホットスタート）
 *   ・複数スクリプト間で衝突しないように名前を分離
 *
 * 引数：
 *   mainFilePath : 実行対象スクリプトのフルパス
 *   mainFunc     : 実行したい main 関数
 *
 * ------------------------------------------------------------
 */
function runMain(mainFilePath, mainFunc)
{
    ...
    // スクリプト固有キー生成
    // =========================================================
    // ファイル名から記号を除去して安全なキーに変換
    // → 複数スクリプト間でグローバル衝突を防ぐ
    var scriptName = File(mainFilePath).name.replace(/\W/g, "_");

    // evalFile済みかどうかのフラグ
    var FLAG = "__RUNMAIN_LOADED__" + scriptName;

    // main関数を保持するグローバルキー
    var MAIN = "__MAIN__" + scriptName;
    ...
    if ($.engineName !== "main")
    {
        // BridgeTalkでIllustratorのmainエンジンに処理を送る
        var bt = new BridgeTalk();
        bt.target = BridgeTalk.appSpecifier;

        // -----------------------------------------------------
        // BridgeTalkで送るコード（文字列として実行される）
        // -----------------------------------------------------
        bt.body =
            '#target illustrator;\n' +
            '#targetengine "main";\n' +
    ...
    // mainFuncが関数として渡されている場合
    if (typeof mainFunc === "function") {

        // -----------------------------------------------------
        // グローバルにmain関数を登録
        // → 次回以降、BridgeTalkから直接呼び出せる
        // -----------------------------------------------------
        $.global[MAIN] = mainFunc;
